...
- Tokens are the custom properties on `:root` in `base.css`, named as in `DESIGN.md` (`--karbon`, `--garis`, ...). Use a token, never a raw hex; the receipt's black in `print.css` is the one exception.
- One file per concern (`base`, `controls`, `table`, `shell`, `home`, `pos`, `nota`, `payment`, `stamp`, `print`). The 200-line limit applies to CSS too (not lint-enforced; checked in review).
